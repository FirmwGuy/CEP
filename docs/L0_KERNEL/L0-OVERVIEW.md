...
* **Episodic Enzyme Engine (E3)** that runs long-lived workloads deterministically, with the ability to promote episodes from threaded RO slices to cooperative RW slices and demote them back without breaking replay or budget accounting.
...
Layer 0’s persistence service (CPS) expects deterministic flat serializer frames and CAS blobs. The fixture workflow lives under `fixtures/cps/` and is exercised by `/CEP/serialization/flat_payload_ref_fixtures` plus `/CEP/cps/replay/*`. Regenerate fixtures by running those tests with `CEP_UPDATE_PAYLOAD_REF_FIXTURES=1`; commits must include both the refreshed frames and blobs so replay harnesses stay hermetic. Metrics such as `cas_hits`, `cas_miss`, `cas_lat_ns`, and the cache policy fields (`hist_ram_bt`, `hist_ram_v`, `ram_quota`, `snapshot_ro`) are republished after every lookup, ensuring `/data/persist/<branch>` reflects cache behaviour even on read-heavy beats. Refer to `docs/L0_KERNEL/design/L0-DESIGN-CPS.md` (Fixture & Replay Workflow) for the rationale and to `docs/L0_KERNEL/L0-INTEGRATION-GUIDE.md` (§2.5) for the exact regeneration steps.

Today every serializer emission runs through the async fabric. `cep_flat_stream_emit_branch_async()` buffers each branch frame during Capture, registers `begin`/`write`/`finish` requests with `cep_io_reactor`, and publishes completions back into CPS and federation via `/rt/ops/<oid>/io_req`. CPS will not advance `ist:store` until the reactor signals success, so beats remain deterministic even when persistence backs up; overruns emit `persist.async`/`persist.async.tmo` CEI. The default reactor uses a portable worker-thread shim so targets without epoll/kqueue/IOCP continue to work, while `/rt/analytics/async/(shim|native)` plus `tp_async_unsp` CEI facts expose when the shim handles requests. When operators import read-only snapshots via `op/br_snapshot`, the branch controller seals the branch (`policy_mode="ro_snapshot"`), publishes `snapshot_ro=1`, and `persist.snapshot` CEI facts explain the transition—these branches skip automatic flushes and rely on lazy hydration + eviction metadata to stay observable.

### Cache policies, lazy hydration, and snapshots
Branch controllers publish everything the cache layer is doing under `/data/persist/<branch>`. The `config/` dictionary exposes the active `policy_mode`, `flush_every`, `flush_shdn`, `allow_vol`, scheduled beats, and the eviction knobs (`hist_ram_bt`, `hist_ram_v`, `ram_quota`, `snapshot_ro`). The sibling `branch_stat/` tree reports the cache horizon (`cache_bt`, `cache_ver`, `cache_bytes`) alongside dirtiness counters so tooling can watch the cache shrink or grow without poking controller internals.

Lazy-load branches advertise `policy_mode="lazy_load"` and stay metadata-only at boot. `cep_branch_registry_bind_existing_children()` records those DTs in a lazy catalog instead of instantiating controllers, and the first reader that calls `cep_runtime_track_data_branch()` claims the entry, registers a controller, and hydrates data through CPS on demand. That handoff keeps `/data/persist/<branch>/config` accurate—the controller flips `policy_mode` to `lazy_load` and continues publishing `hist_ram_*` targets so external monitors see both the intent and the current cache horizon.

Eviction honours both history windows (`hist_ram_bt`, `hist_ram_v`) and byte ceilings (`ram_quota`). `cep_branch_controller_apply_eviction()` trims payload/store histories accordingly, recomputes the cached beat/version/byte counts, and emits `persist.evict` CEI whenever beats fall out of cache because a window or quota bit. Snapshot branches skip scheduled flushes but still update these stats so you can tell how much of an imported read-only snapshot remains resident.

`op/br_snapshot` is the operator surface for sealing a branch. The helper calls `cep_branch_controller_enable_snapshot_mode()`, which seals the tree (`cep_branch_seal_immutable`), switches `policy_mode` to `ro_snapshot`, writes `snapshot_ro=1` under `/config`, and emits a `persist.snapshot` CEI fact. Snapshot branches refuse further mutations (attempts fail immediately) yet stay observable thanks to the lazy-hydrate catalogue and eviction metrics.

**Q&A**
- *How do I know when lazy-load hydration occurred?* Watch `/data/persist/<branch>/branch_stat/cache_bt` alongside `persist.evict`: the cache horizon only advances once hydration has happened, and the lazy catalog entry disappears once claimed.
- *Where do I verify eviction limits?* Read `/data/persist/<branch>/config/hist_ram_*` and `/config/ram_quota` for the target window, then compare to `/branch_stat/cache_*` and any `persist.evict` CEI facts to confirm the controller is trimming as expected.
...
Async I/O brings new observability to this manager. Each mount publishes `async_pnd`, `async_shm`, and `async_nat` counters under `/net/telemetry/<peer>/<mount>/` so operators can see how many requests are inflight, how many completions ran through shim threads, and how many were satisfied by provider-native async hooks. The same data lands in `/rt/analytics/async/(shim|native)/<provider>/<mount>/jobs_total`, letting tooling subscribe to a single runtime branch when correlating OPS dossiers or CEI facts with cluster health.
...
// register here (activation happens either immediately or next beat, deterministically)
...
### Technical details
- Each cell carries a `veiled` flag (previously the “hidden” bit). Default helpers such as `cep_cell_first`, `cep_cell_find_by_name`, and the shallow/deep traversal APIs consult this flag through `cepVisibilityMask`, so callers only see unveiled data unless they explicitly request otherwise.
...
- **Cache policy visibility:** CPS mirrors cache metadata under `/data/persist/<branch>/config` (policy_mode, flush knobs, `snapshot_ro`, `hist_ram_*`, `ram_quota`) and `/branch_stat` (`cache_*`, `dirty_*`). Snapshot branches (`op/br_snapshot`) emit `persist.snapshot` CEI facts and refuse new mutations; eviction telemetry surfaces through `persist.evict`. Tooling should use these fields instead of digging into controllers directly.
...
- **HANDLE/STREAM support:** Stream/handle payload helpers route through `cep_cell_stream_read|write|map`. VALUE/DATA use the inline/history helpers, while HANDLE/STREAM delegate to library adapters with intent/outcome journals and staged commits. Direct `cep_cell_data()` calls still return `NULL` for HANDLE/STREAM so integrations go through the stream APIs.
...
- **How do I confirm a feature exists today versus being on the roadmap?** Check `docs/DOCS-ORIENTATION-GUIDE.md`; planned entries (like Raw Traversal Helpers) are tagged so you do not assume finished behaviour.
