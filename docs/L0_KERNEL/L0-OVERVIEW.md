...
For multi-beat work, the **Episodic Enzyme Engine (E³)** builds on this loop. Episodes queue through the cooperative executor (`cep_executor_submit_ro()` today, threaded backends later), carry per-slice CPU/IO budgets, and surface state transitions as `op/ep` dossiers so tooling can await `ist:run`, `ist:await`, or `ist:cxl`. Mutation helpers now consult the TLS context (`cep_ep_require_rw`, `cep_ep_check_cancel`, `cep_ep_account_io`) so read-only slices stay safe and CEI records guard or budget violations. See `docs/L0_KERNEL/topics/E3-EPISODIC-ENGINE.md` for the full lifecycle and migration guidance from Rendezvous.
