...
- **Runtime default policy.** `cepEpRuntimeState` seeds a runtime-owned default execution policy (RO profile plus configured CPU/IO budgets). Executor submissions inherit it whenever a policy field is zero, so multi-runtime setups and tests can override defaults without relying on static macros.
