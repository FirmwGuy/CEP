...
### Checksums
- Frames advertise the checksum algorithm in both the optional compression container header and the frame trailer. Writers default to the IEEE CRC-32 polynomial via zlib; set `CEP_CRC32C_MODE=castagnoli` (or `CEP_SERIALIZATION_FLAT_CHECKSUM=crc32c`) to request CRC32C when the CPU exposes SSE4.2/ARM CRC32 instructions, and force `CEP_SERIALIZATION_FLAT_CHECKSUM=crc32` to downgrade per-frame when federation peers reject CRC32C. Emitters automatically fall back to IEEE CRC32 when hardware support is missing, and readers always verify records with the algorithm recorded in the frame so policies can mix without desyncing CRC expectations.
...
By default, every Layer 0 node emits flat frames. Federation, replication, archives, and debug tooling all expect this format, so when you update the serializer or add a record type, update the spec and this topic plus the test cases listed above.

### Secured Payload Encoding
