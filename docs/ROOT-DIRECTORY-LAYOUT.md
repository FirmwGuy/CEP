...
- `/sys` – core counters, configuration toggles, and name tables the kernel reads each beat. The `/sys/state/<scope>` dictionary also records lifecycle readiness (`status`, `ready_beat`) and teardown metadata (`td_beat`) for kernel subsystems. The kernel now seeds `/sys/security` with deterministic dictionaries (`enclaves`, `edges`, `gateways`, `branches`, `defaults`, `env/{prod,staging,dev}`) so Access & Execution Control policies have a canonical tree before packs start publishing rules.
...
- `/data` – durable state, promoted from `/rt/.../stage` at the commit edge of a beat. Optional upper-layer packs can pre-create their own namespaces here (for example, a coherence pack might reserve `/data/coh`). The kernel now seeds `/data/mailbox/impulses` as a control-plane mailbox where paused impulses park until `op/resume` drains them. Packs that participate in Enclave enforcement reserve `/data/<pack>/policy/security/` (under their own namespace) to store approved pipeline specs and attestation evidence.
