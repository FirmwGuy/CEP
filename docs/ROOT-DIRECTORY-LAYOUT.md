...
- `/sys` – core counters, configuration toggles, and name tables the kernel reads each beat. The `/sys/state/<scope>` dictionary also records lifecycle readiness (`status`, `ready_beat`) and teardown metadata (`td_beat`) for kernel subsystems. The kernel seeds `/sys/security` with deterministic dictionaries (`enclaves`, `edges`, `gateways`, `branches`, `defaults`, `env/{prod,staging,dev}`) so Access & Execution Control policies have a canonical tree before packs start publishing rules.
