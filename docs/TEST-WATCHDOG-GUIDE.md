...
- `test_watchdog_create(seconds)` arms a background thread that calls `_Exit()` once the negotiated timeout elapses; Layer 0 Stage E suites default to 240 s but you can override the budget with the `timeout=` Munit parameter.
...
- The shared `test_stagee_watchdog_setup/tear_down` fixture arms watchdogs for `/CEP/heartbeat/bootstrap`, `/CEP/organ/sys_state`, and `/CEP/organ/rt_ops`, ensuring they signal completion before teardown.
...
- **Do I have to touch every test to use the watchdog?** No—Stage E suites already share a fixture. For other suites, call `test_watchdog_create()` in your setup and `test_watchdog_destroy()` in teardown to adopt the same pattern.
