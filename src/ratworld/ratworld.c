                                          ...
                                          uint64_t *rng_state)
{
  ...
  while (attempts < total * 2) {
    ...
    if (low->tiles[low_idx].type == RATWORLD_TILE_FLOOR &&
      ...
      /* Stairs go up from the lower floor and down from the upper floor so
         traversal and tick logic can move between floors. */
      low->tiles[low_idx].type = RATWORLD_TILE_STAIR_UP;
      high->tiles[high_idx].type = RATWORLD_TILE_STAIR_DOWN;
      return;
    }
    ++attempts;
  }
