/*
 *  Stream fuzzing drives randomized read/write chunks through the stdio and
...
MunitResult test_streams_stdio_randomized(const MunitParameter params[], void* fixture) {
    ...
    cepCell library;
    CEP_0(&library);
    cep_stdio_library_init(&library, CEP_DTS(CEP_ACRO("CEP"), CEP_WORD("stdio")));
    ...
    uint8_t payload[256];
    size_t total = (size_t)munit_rand_int_range(48, 192);
    munit_rand_memory(total, payload);

    size_t offset = 0;
    while (offset < total) {
        size_t chunk = (size_t)munit_rand_int_range(4, 24);
    ...
    munit_assert_memory_equal(slice_size, payload + slice_offset, buffer);
...
MunitResult test_streams_zip_randomized(const MunitParameter params[], void* fixture) {
    ...
    munit_assert_true(cep_zip_library_open(&library,
                                           CEP_DTS(CEP_ACRO("CEP"), CEP_WORD("zip")),
                                           archive_path,
    ...
    for (size_t i = 0; i < entry_count; ++i) {
        ...
        CEP_0(&stream);
        cep_zip_stream_init(&stream,
                            CEP_DTS(CEP_ACRO("CEP"), CEP_WORD("zip_reader")),
                            &library,
                            &resource);
